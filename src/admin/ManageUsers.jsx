...
function ManageUsers() {
  ...
  const handleBlockUnblock = async (userId, isBlocked) => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      await UpdateUserBlockStatus(userId, newStatus, token);
      console.log("Updating user", userId, "to", newStatus);
      const res = await UpdateUserBlockStatus(userId, newStatus, token);
                          ...
                          <div className="flex justify-end gap-1">
                            {user.role === "user" && (
                              <>
                                ...
                                {/* Delete */}
                                <button
                                  className={`${user.isDeleted===true?"text-red-50 opacity-0":"text-red-600"} hover:text-red-900 p-2 rounded-full hover:bg-red-50 transition-colors duration-200 ${
                                    deletingId === user.id
                                      ? "opacity-50 cursor-not-allowed"
                                      : ""
                                  }`}
