  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        const data2 = await GetAllProducts(0,100);
        setItem(data);
        setItem2(data2);
      } catch (err) {
    ...
    <div className="mt-10">
      ...
      <div className="min-h-[200px]">
        {loading ? (
          ...
          <div className="grid gap-1 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 justify-items-center">
            {item.map((e) => (
              <Card
                key={e?.id}
                id={e?.id}
